...
(function(){
	...
	function initRadio(){
		...
		radio.addListener(radio.EVT_RADIO_STARTUP,function(evt){
			/*
			if(gBrowser.contentDocument.location.href=="http://douban.fm/"){
				var selecid=gBrowser.contentDocument.getElementsByClassName("channel selected")[0].getAttribute("cid");
				if(selecid!=null) {
					radio.setRadioChannel(selecid,'0');
					
					alert(gBrowser.contentDocument.getElementsByClassName("channel selected")[0].childNodes[1].innerHTML.match("^.+<span").replace("<span",""));
				}
				*/
	...
	function initMenu(){
        ...
        //Steve edit
		ce('menuseparator', p)
		 ce('menuitem', p, {id:'dbr_mi_love'},{command:loveIt})
		 ce('menuitem', p, {id:'dbr_mi_openartist'},{command: openArtistPage})
		 ce('menuitem', p, {id:'dbr_mi_openalbum'},{command: openAlbumPage})
       ce('menuitem', p, {id:'dbr_mi_hate'},{command:hateIt})
